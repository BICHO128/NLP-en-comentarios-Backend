...
from documentacion.evaluacion.models import Docente, Curso, Evaluacion, Calificacion, Comentario, Criterio, User
...
def generar_excel_admin():
    ...
    for ev in evaluaciones:
        usr        = User.query.get_or_404(ev.docente.docente_id)
    ...
    pd.DataFrame(filas_cal).to_excel(
        writer, sheet_name="Resumen Evaluaciones", index=False
    )
...
def generar_pdf_docente(docente_id, curso_id):
    ...
    if curso not in docente.cursos:
        return jsonify({"error": "El curso no pertenece al docente"}), 400

    # Traer evaluaciones
    evs = Evaluacion.query.filter_by(docente_id=docente_id, curso_id=curso_id).all()
    if not evs:
        return jsonify({"error": "No hay evaluaciones registradas"}), 404
    ...
    usuario = User.query.get_or_404(docente.docente_id)
    ...
    pdf.seek(0)
    fname = f"reporte_{usuario.first_name}_{usuario.last_name}_{curso.nombre.replace(' ', '_')}.pdf"
